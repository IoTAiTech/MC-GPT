...
Local Claude, Codex and Grok must be the user-local CLIs (`~/.local/bin`, then `~/.grok/bin`). Grok Build flag order is `grok -p "<prompt>" --output-format json`. See [local CLI seats](docs/local-cli-seats.md) and [minimum change gate](docs/minimum-necessary-change-gate.md).
