...
from .completion_evidence import read_artifact, verify_completion
...
def _evidence_integrity(user_home:Path,records:list[dict[str,Any]])->tuple[bool,list[str]]:
    ...
    for row in records:
        ...
        try:
            if sha256_file(path, allowed_roots=[user_home, Path.cwd().resolve()], max_bytes=4*1024*1024)!=row["artifact_sha256"]:
                findings.append(f"evidence-hash-mismatch:{row['id']}")
        except (OSError, ValueError):
            findings.append(f"evidence-unreadable:{row['id']}")
    return not findings,findings


def audit_task(user_home:Path,task_id:str,*,record:bool=True,connection:Any=None,transition:dict[str,Any]|None=None)->dict[str,Any]:
    if connection is not None and record:
        raise ValueError("caller-transaction-audit-must-be-read-only")
    owns_connection=connection is None
    conn=connect_read(user_home) if owns_connection else connection
    if conn is None: raise ValueError("Task not found")
    task=one(conn,"SELECT * FROM tasks WHERE id=?",(task_id,))
    if not task:
        if owns_connection: conn.close()
    ...
    attempts=rows(conn,"SELECT * FROM attempts WHERE task_id=? ORDER BY rowid",(task_id,))
    evidence=rows(conn,"SELECT * FROM evidence WHERE task_id=? ORDER BY rowid",(task_id,))
    tests=rows(conn,"SELECT rowid AS ledger_sequence,* FROM test_results WHERE task_id=? ORDER BY rowid",(task_id,))
    ...
    completion=verify_completion(user_home,conn,task,transition=transition)
    chain=verify_event_chain(user_home,connection=conn)
    if owns_connection: conn.close()
    # The latest attempted run supersedes historical tests, even if it never
    # reached its test stage. Within one run, failure supersedes earlier pass.
    run_id=completion.get("run_id") or (attempts[-1].get("run_id") if attempts else (tests[-1].get("run_id") if tests else None))
    evidence=[row for row in evidence if json.loads(row["metadata_json"]).get("run_id")==run_id]
    evidence_ok,evidence_findings=_evidence_integrity(user_home,evidence)
    substantive={row["provider"] for row in contributions if row["run_id"]==run_id and row["status"]=="pass" and row["stage"] in {"plan","critique","final-review"}}
    current_tests=[row for row in tests if row.get("run_id")==run_id]
    latest_by_tier={row["tier"]:row for row in current_tests}
    test_by_tier={tier:row for tier,row in latest_by_tier.items() if row["decision"]=="pass" and row["exit_code"]==0 and not row.get("skipped")}
    final_reviews=[row for row in final_reviews if row.get("run_id")==run_id]
    ...
    gates={
        ...
        "completion_evidence_bound":completion["decision"]=="pass",
        ...
        "mandatory_tests_pass":bool(current_tests) and (all(tier in test_by_tier for tier in MANDATORY_TIERS) if governed else len(test_by_tier)==len(latest_by_tier)),
    ...
    payload={"schema":"iot-ai.task-audit.v2","task_id":task_id,"run_id":run_id,"decision":decision,"gate_score":score,"gates":gates,"findings":findings,"case_counts":case_counts,"substantive_providers":sorted(substantive),"mandatory_tiers":list(MANDATORY_TIERS),"excel_sha256":excel_sha,"event_chain":chain,"created_at":utc_now()}
