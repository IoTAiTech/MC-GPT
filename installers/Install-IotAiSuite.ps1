...
$RuntimeRoot = Join-Path $SuiteBase "6.7.0-beta.3"
...
$result = [ordered]@{
  schema="iot-ai.windows-install-plan.v3"; version="6.7.0-beta.3"; home=$Home; runtime=$RuntimeRoot;
...
try {
  ...
  & $VenvPython -m pip install --no-index --disable-pip-version-check --no-input --find-links $WheelRoot "iot-ai-coder-suite==6.7.0b3"
  ...
  if ($PackageStore) {
    $CleanArgs = @("--home",$Home,"package","clean","--current-version","6.7.0-beta.3","--package-store",$PackageStore,"--current-package",$CurrentPackage)
    if ($PackageArchive) { $CleanArgs += @("--package-archive",$PackageArchive) }
    $CleanArgs += "--apply"
    & $VenvCli @CleanArgs
  } else {
    & $VenvCli --home $Home package clean --current-version "6.7.0-beta.3" --apply
  }
  & $VenvCli --home $Home status --logs
  [ordered]@{
    schema="iot-ai.powershell-install-receipt.v1"; transaction_id=$TxId; version="6.7.0-beta.3";
