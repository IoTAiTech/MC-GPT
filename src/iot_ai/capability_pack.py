...
def build_pack(spec: Mapping[str, Any], output: Path) -> dict[str, Any]:
    ...
    return {
        "decision": "pass",
        "path": str(output),
        "sha256": sha256_file(output, allowed_roots=[output.parent.resolve()], max_bytes=None),
        "name": name,
        "version": version,
        "classification": classification,
        "boundaries": boundaries,
...
def verify_pack(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "path": str(path),
        "sha256": sha256_file(path, allowed_roots=[path.parent.resolve()], max_bytes=None) if path.is_file() else None,
        "errors": errors,
    }
