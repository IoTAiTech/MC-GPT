...
"""Public-safe fixtures — never embed contiguous private IP/path/key literals."""
...
class ProductBoundaryTests(unittest.TestCase):
    def test_blocks_product_store_without_real_host_details(self):
        private_path = _join("/", "ho", "me", "/", "operator", "/", "product", "/", "pmd", "/", "data", "/", "tasks.db")
    ...
    def test_blocks_fcc_and_hid_markers(self):
        with self.assertRaises(ProductBoundaryError):
            assert_not_product_database(_join("/", "var", "/", "lib", "/", "aiiot_fcc", "/", "state.db"), context="test")
        with self.assertRaises(ProductBoundaryError):
            assert_not_product_database(_join("C:", "\\", "sandbox", "\\", "hid", "\\", "data", "\\", "agents.db"))

    def test_allows_suite_workspace(self):
        assert_not_product_database(_join("/", "tmp", "/", "iot-ai-home", "/", "state", "/", "tasks.db"))


class ExportGateTests(unittest.TestCase):
    def test_redacts_private_classes_without_real_fleet_literals(self):
        private_ip = _join("192", ".", "16", "8", ".", "77", ".", "88")
        private_path = _join("/", "ho", "me", "/", "operator", "/", "private", "/", "report.md")
    ...
    def test_brief_has_semantic_participant_summary(self):
        payload = {
            ...
            "meeting": {
                ...
                "substantive_seats": 1,
                "requested_seats": 2,
                "synthesis": "Use authenticated API; do not share databases.",
                "contributions": [
                    {
                        "kind": "opinion",
                        "seat": "claude",
                        ...
                        "model_requested": "m1",
                        "model_served": "m1",
                        "text": "Use a versioned API and keep PMD as a client.",
                    },
                    {
                        "kind": "opinion",
                        "seat": "codex",
